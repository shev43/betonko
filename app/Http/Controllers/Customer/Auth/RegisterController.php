    ...
    class RegisterController extends Controller {
        ...
        public function registerVerified(Request $request) {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'phone' => ['required', 'max:255'], 'accept' => ['required'],]);
            if($validator->fails()) {
                return response()->json([
                    'message' => __('Щось пішло не так :('), 'errors' => $validator->validate()], 422);
            }
        ...
        public function store(Request $request) {
            ...
            if($authLoginCode->code !== $request_code) {
                return response()->json([
                    'message' => __('Щось пішло не так :('),
                    'errors' => ['code.exists' => __('Код невірний: ' . $authLoginCode . ' phone: ' . $this->phone . ' code: ' . $request_code)]], 422);
            }
