    ...
    <div class="card">
        <div class="container-fluid">
            <div class="row content-min-height">
                ...
                <div class="col">
                    <div class="card-body">
                        <form action="{{ route('amicms::business.factories.store', ['business_id'=>$business_id]) }}" method="post" enctype="multipart/form-data">
                            @csrf
                                            ...
                                            <td class="py-3">
                                                <select class="selectpicker" name="contacts[]" id="contacts" data-style="form-control @if($errors->has('contacts')) noty_type_error @endif" data-title="Виберіть контактну особу" multiple>
                                                    @foreach($contacts as $contact)
                                                        <option value="{{ $contact->id }}" {{in_array($contact->id, old("contacts") ?: []) ? "selected": ""}}>{{ $contact->name }}</option>
                                                    @endforeach
                                                </select>
