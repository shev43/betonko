    ...
    <main class="main">
        <section class="heroslide">
            <figure class="heroslide-video">
                <div class="mobile" style="background-image: url({{ asset('img/layout/homepage/bg.jpg') }});"></div>
            </figure>
            <img class="heroslide-truck" src="{{ asset('img/layout/homepage/truck.png') }}" alt="Бетоновоз">
            <div class="container position-relative">
                ...
                @if(Auth::guard('client')->check())
                    <a href="{{ route('customer::catalog.index', ['lang'=>\Illuminate\Support\Facades\App::getLocale()]) }}" class="btn heroslide-btn">Замовити</a>
                @elseif(Auth::guard('business')->check())
                    <a href="{{ route('business::catalog.index', ['lang'=>\Illuminate\Support\Facades\App::getLocale()]) }}" class="btn heroslide-btn">Замовити</a>
                @else
                    <a href="{{ route('frontend::catalog.index', ['lang'=>\Illuminate\Support\Facades\App::getLocale()]) }}" class="btn heroslide-btn">Замовити</a>
                @endif
            </div>
            <div class="heroslide-wave">
            ...
            <div class="container">
                <div class="row justify-content-center justify-content-md-start">
                    <div class="col-10 col-md-5 col-lg-4 mr-md-auto mx-lg-auto">
                        <div class="s_benefits-item s_benefits-item--buyer">
                            <div class="position-relative">
                                ...
                                <div class="text-center text-md-left">
                                    @if(Auth::guard('client')->check())
                                        <a href="{{ route('customer::catalog.index', ['lang'=>app()->getLocale()]) }}" class="btn btn-border_light">Придбати</a>
                                    @else
                                        <a href="{{ route('frontend::catalog.index', ['lang'=>app()->getLocale()]) }}" class="btn btn-border_light">Придбати</a>
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-10 col-md-5 col-lg-4 ml-md-auto mx-lg-auto">
                        <div class="s_benefits-item s_benefits-item--seller">
                            <div class="position-relative">
                                ...
                                <div class="text-center text-md-left">
                                    <a href="#" data-toggle="modal" data-target="#authModal" data-auth-action="register" class="btn btn-border_dark">Стати партнером</a>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="s_faq-item-content">
                                    <p>Платформа є безкоштовною для всіх користувачів! Якщо ви продавець і бажаєте збільшити свої продажі то для вас ми пропонуємо пакет "Бетонко Експерт" - це набір додаткових функцій для виділення вашого продукту серед інших. Детальніше про пакет
                                        @if(Auth::guard('business')->check())
                                            <a href="{{ route('business::subscription.index', ['lang'=>app()->getLocale()]) }}">"Бетонко Експерт"</a>
                                        @else
                                            <a href="{{ route('frontend::subscription.index', ['lang'=>app()->getLocale()]) }}">"Бетонко Експерт"</a>
                                        @endif
                                        тут.</p>
                                </div>
                            </div>
