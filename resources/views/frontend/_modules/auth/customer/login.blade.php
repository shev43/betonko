<div class="form-step step-1">
    <form method="post" action="{{ route('customer::profile.login.sms', ['lang'=>app()->getLocale()]) }}" id="send-login-sms">
        @csrf
        ...
        <div class="form-group">
            ...
            <button type="button" class="btn btn-border_dark go-to-register">Зареєструватися</button>
        </div>
    </form>
</div>

<div class="form-step step-2" style="display: none">
    <form method="post" action="{{ route('customer::profile.login.send', ['lang'=>app()->getLocale()]) }}" id="send-login">
        @csrf

        <input type="hidden" name="phone">
        <input type="hidden" name="code">
        @if(!Auth::guard('client')->check())
            @if(isset($technic) && (request()->routeIs('frontend::order.index') || request()->routeIs('business::order.index')))
