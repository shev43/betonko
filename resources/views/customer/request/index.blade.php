                        ...
                        <div class="customer_cabinet-item @if($order->status == 'canceled' || ( (count($order->offers) > 0) && $order->offers[0]['status'] == 'canceled')) disabled @endif">
                            <div class="row">
                                <div class="col-lg-5">
                                    ...
                                    @if($order->is_tender == 1 && $order->status == 'new')
                                    ...
                                    @if(count($order->offers) > 0)
                                        ...
                                        @if($order->offers[0]['status'] !== 'canceled')
                                            @if($order->status == 'new' && $order->offers[0]['status'] == 'new')
                                                <div class="customer_cabinet-item-status" style="height: 36px;">
                                                    <svg class="icon">
                                                        <use xlink:href="#icon-clock"></use>
                                                    </svg>
                                                    <b>Є зустрічна пропозиція</b>
                                                </div>
                                        ...
                                        <div class="customer_cabinet-item-status mt-4">
                                            @if($order->offers[0]['status'] !== 'canceled')
                                                ...
                                                @if($order->is_tender == 0)
                                                    <a href="{{ route('customer::request.offer.view', ['lang'=>app()->getLocale(), 'offer_id'=>$order->offers[0]->id ]) }}" class="btn btn-default customer_cabinet-item-btn">
                                                        <span>Детальніше</span>
                                                    </a>
                                                @else
                                                    <a href="{{ route('customer::request.offers.index', ['lang'=>app()->getLocale(), 'order_id'=>$order->id ]) }}" class="btn btn-default customer_cabinet-item-btn">
                                                        <span>{{ count($order->offers) }} пропозицій</span>
                                                    </a>
                                                @endif
                                            @endif
                                ...
                                <div class="col-lg-7">
                                    <div class="parameters">
                                        ...
                                        <div class="position-absolute" style="bottom: 15px;left: 35px;">
                                            <div class="mt-2 mb-2"><a class="" href="{{ route('customer::request.create-duplicate', ['lang'=>app()->getLocale(), 'order_id'=>$order->id]) }}">
                                                    <svg class="icon">
                                                        <use xlink:href="#icon-25"></use>
                                                    </svg>
                                        ...
                                        @if($order->status == 'new')
                                            <div class="position-absolute" style="bottom: 15px;right: 15px;">
                                                <button class="btn btn-line btn-fill" href="#" data-href="{{ route('customer::request.canceled', ['lang'=>app()->getLocale(), 'order_id'=>$order->id]) }}" data-toggle="modal" data-target="#confirm-canceled">
                                                    <svg class="icon">
                                                        <use xlink:href="#icon-clear"></use>
                                                    </svg>
                                                </button>
