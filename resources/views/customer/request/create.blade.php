...
@section('content')
    <nav class="container mt-3" aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            @if(Auth::guard('client')->check())
                <li class="breadcrumb-item"><a href="{{ route('customer::company.view', ['lang'=>app()->getLocale(), 'company_id'=>$product->business->id]) }}">{{ $product->business->name }}</a></li>
            @else
                <li class="breadcrumb-item"><a href="{{ route('frontend::company.view', ['lang'=>app()->getLocale(), 'company_id'=>$product->business->id]) }}">{{ $product->business->name }}</a></li>
            @endif

            @if(Auth::guard('client')->check())
                <li class="breadcrumb-item"><a href="{{ route('customer::catalog.view', ['lang'=>app()->getLocale(), 'factory_id'=>$product->factory->id]) }}">{{ $product->factory->name }}</a></li>
            @else
                <li class="breadcrumb-item"><a href="{{ route('frontend::catalog.view', ['lang'=>app()->getLocale(), 'factory_id'=>$product->factory->id]) }}">{{ $product->factory->name }}</a></li>
            @endif
            <li class="breadcrumb-item active" aria-current="page">Створення заявки</li>
        </ol>
    </nav>
    <main class="main page">
        <div class="container">
            <form id="customerRequest" action="{{ route('customer::request.store', ['lang'=>app()->getLocale()]) }}" method="post" class="order-form">
                ...
                <input name="seller_id" type="hidden" value="{{ $product->business->user_id ?? null }}">
                <input name="factory_id" type="hidden" value="{{ $product->factories_id ?? null }}">
                <input name="mark" type="hidden" value="{{ $product->mark ?? null }}">
                <input name="class" type="hidden" value="{{ $product->class ?? null }}">
                <input name="frost_resistance" type="hidden" value="{{ $product->frost_resistance ?? null }}">
                <input name="water_resistance" type="hidden" value="{{ $product->water_resistance ??  null }}">
                <input name="mixture_mobility" type="hidden" value="{{ $product->mixture_mobility ?? null }}">
                <input name="winter_supplement" type="hidden" value="{{ $product->winter_supplement ?? null }}">
                    ...
                    <div class="row justify-content-between">
                        ...
                        <div class="col-md-5">
                            ...
                            <div class="order-price">
                                ...
                                @if (!empty($product))
                                    <div class="title"><span>{{ (int)$product->price }}</span> грн.</div>
                                    <input type="hidden" name="min_price" value="{{ intval($product->price) }}">
                                    <input type="hidden" name="max_price" value="{{ intval($product->price) }}">
                                @else
                                    <div class="range">
                ...
                <section class="order-delivery">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            @include('frontend._modules.map-form', ['objects'=>[], 'target'=>'form#customerRequest input[name=address]'])
                        </div>
                        <div class="col-lg-4 offset-lg-1 col-md-6">
                                ...
                                <div class="row one-active-checkbox">
                                    <div class="col-6 col-md-12 col-lg-6">
                                        <div class="form-check">
                                            <input class="d-none"
                                                ...
                                                {{ old('type_of_delivery', '') == 'self' ? 'checked' : '' }}
                                            >
                                            <div class="form-check">
                                                <label for="type-self">
                                                    <svg>
                                    ...
                                    <div class="col-6 col-md-12 col-lg-6">
                                        <input class="d-none"
                                               ...
                                               value="business"
                                               data-field="type_of_delivery"
                                            {{old('type_of_delivery', 'business') == 'business' ? 'checked' : ''}}
                                        >
                                        <div class="form-check">
                                    ...
                                    <input type="hidden" name="type_of_delivery" id="type_of_delivery"
                                           value="{{old('type_of_delivery', 'business')}}">
                            ...
                            <div class="form-group address-block {{old('type_of_delivery', '') == 'self' ? 'disabled' : ''}}">
                                ...
                                <input type="text" class="form-control autocomplete {{$errors->has('address') ? 'alert-danger' : ''}}"
                                    ...
                                    {{old('type_of_delivery', '') == 'self' ? 'disabled' : ''}}
                                    {{old('type_of_delivery', '') == 'self' ? '' : ' required'}}
                                >
