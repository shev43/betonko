    ...
    <main class="main page">
        <div class="container">
            ...
            <section class="seller_cabinet-settings-persons">
                <h2 class="title text-center">Редагувати продукт</h2>

                <form action="{{ route('business::setting.product.update', ['lang'=>app()->getLocale(), 'product_id'=>$product->id]) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="row justify-content-center">
                                ...
                                <div class="">
                                    <select name="mark" id="mark"
                                        ...
                                        @foreach(Config::get('product.mark') as $value => $mark)
                                            <option value="{{$value}}"
                                                {{old('mark') == $value || $product->mark == $value ? 'selected="selected"' :''}}>
                                                {{$mark}}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="">
                                    <select name="class" id="class"
                                        ...
                                        @foreach(Config::get('product.class') as $value => $class)
                                            <option value="{{$value}}"
                                                {{old('class') == $value || $product->class == $value ? 'selected="selected"' :''}}>
                                                {{$class}}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="">
                                    <select name="water_resistance" id="water_resistance"
                                        ...
                                        @foreach(Config::get('product.water_resistance') as $value => $waterResistance)
                                            <option value="{{$value}}"
                                                {{old('water_resistance') == $value || $product->water_resistance == $value ? 'selected="selected"' :''}}>
                                                {{$waterResistance}}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="">
                                    <select name="winter_supplement" id="winter_supplement"
                                        ...
                                        @foreach(Config::get('product.winter_supplement') as $value => $winterSupplement)
                                            <option value="{{$value}}"
                                                {{old('winter_supplement') == $value || $product->winter_supplement == $value ? 'selected="selected"' :''}}>
                                                {{$winterSupplement}}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="">
                                    <select name="mixture_mobility" id="mixture_mobility"
                                        ...
                                        @foreach(Config::get('product.mixture_mobility') as $value => $mixtureMobility)
                                            <option value="{{$value}}"
                                                {{old('mixture_mobility') == $value || $product->mixture_mobility == $value ? 'selected="selected"' :''}}>
                                                {{$mixtureMobility}}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="">
                                    <select name="frost_resistance" id="frost_resistance"
                                        ...
                                        @foreach(Config::get('product.frost_resistance') as $value => $frostResistance)
                                            <option value="{{$value}}"
                                                {{old('frost_resistance') == $value || $product->frost_resistance == $value ?'selected="selected"' :''}}>
                                                {{$frostResistance}}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="col-md-4">
                            <div class="form-group ">
                                ...
                                <input type="number" class="form-control {{$errors->has('price') ? 'is-invalid' : ''}}" name="price" min="1" max="9999" step="1" maxlength="4" value="{{ $product->price }}" placeholder="Виберіть ціну" autocomplete="off"  oninput="if(value.length>4)value=value.slice(0,4)">
                            </div>
                        </div>
                        <div class="col-md-8">
                            <div class="form-group ">
                                ...
                                <select class="selectpicker @error('factories') is-invalid @enderror" id="factories" name="factories" data-style="form-control" title="Виберіть завод">
                                    @foreach($product->factories as $factory_key=>$factory)
                                        @php($selected = '')
                                        @foreach(explode(',', $product->factories_id) as $product_factory)
                                            @if($factory->id == $product_factory)
                                                @php($selected = 'selected')
                                            @endif
                                        @endforeach

                                        <option value="{{ $factory->id }}"
                                            {{old('factories.'.$factory_key.'') == $factory->id ?'selected="selected"' :''}} {{ $selected }}>
                                            {{ $factory->name }}
                                        </option>
                                    @endforeach
                                </select>
