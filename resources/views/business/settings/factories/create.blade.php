    ...
    <main class="main page">
        <div class="container">
            ...
            <section class="seller_cabinet-settings-persons">
                <h2 class="title text-center">Додати завод</h2>

                <div id="businessSettingFactoryPhoto" class="row justify-content-center">
                    <div class="col-lg-3 col-md-4">
                        <div class="form-group text-center">
                            <img class="seller_cabinet-settings-logo" src="{{ asset('img/layout/profile-logo.jpg') }}" data-empty="{{ asset('img/layout/profile-logo.jpg') }}" alt="">
                        </div>
                    </div>
                ...
                <form id="businessFactory" action="{{ route('business::setting.factories.store', ['lang'=>app()->getLocale()]) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="row justify-content-center">
                        <div class="col-lg-8">
                            <div class="form-group">
                                ...
                                <input name="name" type="text" class="form-control @error('name') is-invalid @enderror" id="name" placeholder="Вкажіть назву заводу" value="{{ old('name') }}">
                            </div>
                        </div>
                        ...
                        <div class="col-lg-8">
                            <div class="form-group">
                                ...
                                <select class="selectpicker @error('contacts') is-invalid @enderror" name="contacts[]" data-style="form-control" data-title="Оберіть контактних осіб" multiple>
                                    @foreach($business->contacts as $contact_key=>$contacts)
                                        <option value="{{ $contacts->id }}"
                                            {{old('contacts.'.$contact_key.'') == $contacts->id ?'selected="selected"' :''}}>
                                            {{ $contacts->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-lg-8">
                            <div class="form-group">
                                ...
                                <input name="address" type="text" class="form-control autocomplete @error('address') is-invalid @enderror" id="position" placeholder="Вкажіть розташування заводу" value="{{ old('address') }}">
                            </div>

                            @include('frontend._modules.map-form', ['objects'=>[], 'target'=>'form#businessFactory input[name=address]'])
